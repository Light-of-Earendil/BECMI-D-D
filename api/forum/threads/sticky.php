<?php
/**
 * BECMI D&D Character Manager - Forum Thread Sticky Endpoint
 * 
 * Makes a thread sticky or unsticky (moderator only)
 */
...
try {
    ...
    $isSticky = isset($data['is_sticky']) ? (bool) $data['is_sticky'] : true;
    
    // Get thread
    $thread = $db->selectOne(
        "SELECT thread_id, is_sticky FROM forum_threads WHERE thread_id = ?",
        [$threadId]
    );
    ...
    $db->execute(
        "UPDATE forum_threads 
         SET is_sticky = ?, updated_at = NOW()
         WHERE thread_id = ?",
        [$isSticky ? 1 : 0, $threadId]
    );
    ...
    ], $isSticky ? 'Thread made sticky successfully' : 'Thread unstuck successfully');
    
} catch (Exception $e) {
    error_log('Forum thread sticky error: ' . $e->getMessage());
    error_log('Stack trace: ' . $e->getTraceAsString());
    Security::sendErrorResponse('An error occurred while making thread sticky/unsticky', 500);
}