<?php
/**
 * BECMI D&D Character Manager - Run Map Migration Endpoint
 * 
 * Runs the session map scratch-pad migration
 * Admin/DM only
 */
...
try {
    ...
    // Only allow admins or DMs to run migrations
    // For now, allow any authenticated user (you can restrict this further)
    
    $db = getDB();
    
    $results = [];
    
    // Create session_maps table
    try {
        $db->execute("
        ...
        ");
        $results[] = 'session_maps table created';
    } catch (Exception $e) {
        $results[] = 'session_maps: ' . $e->getMessage();
    }
    
    // Create session_map_drawings table
    try {
        $db->execute("
        ...
        ");
        $results[] = 'session_map_drawings table created';
    } catch (Exception $e) {
        $results[] = 'session_map_drawings: ' . $e->getMessage();
    }
    
    // Create session_map_tokens table
    try {
        $db->execute("
        ...
        ");
        $results[] = 'session_map_tokens table created';
    } catch (Exception $e) {
        $results[] = 'session_map_tokens: ' . $e->getMessage();
    }
    ...
    error_log("MIGRATION ERROR STACK TRACE: " . $e->getTraceAsString());
    Security::sendErrorResponse('Failed to run migration: ' . $e->getMessage(), 500);
}
?>
